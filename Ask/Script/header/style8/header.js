$(function() {
    ...
    if (/android/i.test(navigator.userAgent) || /iphone/i.test(navigator.userAgent)) {
        ...
        slideMenuDesktop($handle)
        slideMenuDesktop($drop)
    }

    //如果沒nav menu
    if($('.header > .nav > .nav-menu').children('li').length < 0 || $('.mobile-menu .nav.top').length < 0) {
        $('body').addClass('nav-hide');
    } else {
        $('body').removeClass('nav-hide');
    }

    // if($('.mobile-menu .nav.top').length) {
    //     $('body').addClass('member-has-login');
    // } else {
    //     $('body').removeClass('member-has-login');
    // }

    if($('.mobile-menu .nav').hasClass("top")) {
        $('body').addClass('member-has-login');
    } else {
        $('body').removeClass('member-has-login');
    }
    ...
    $("body").waypoint(function() {
        ...
        $('body').toggleClass('header--shrink');
    }, {
        offset: -($headerH)
    });

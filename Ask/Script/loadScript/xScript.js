    ...
    $.extend({
        loadScript: function(files) {
            $.map(files, function(file) {
                if (!sessionStorage.getItem(file.src)) {

                    var s = document.createElement('script');
                    s.setAttribute('type', 'text/javascript');
                    s.setAttribute('src', file.src);
                    var head = document.getElementsByTagName('head')[0];

                    if (file.name) {
                        if (file.callback)
                            $.wait_for_script_load(file.name, file.callback);
                        console.log("判斷實例化" + file.name + " 直接載入 :: " + file.src);
                        if (eval("typeof " + file.name) == 'undefined') {
                            if (head) {
                                head.appendChild(s);
                            } else {
                    ...
                    sessionStorage.setItem(file.src, true);

                } else {
                    console.log("已載入過 :: " + file.src + " 檢查實例化後執行")
                    if (file.callback)
                        $.wait_for_script_load(file.name, file.callback);
        ...
        wait_for_script_load: function(name, callback) {

...
{
    public class LogStatisticLabelDAO
    {
        ...
        public static int Delete(IEnumerable<long> ids) {
            ...
            string sql =
                "Delete LogStatisticLabels Where ID In ({0})\n";
        ...
        public static void SetItem(LogStatisticLabelModels item)
        {
            ...
            SQLData.TableObject tableObj = db.GetTableObject("LogStatisticLabels");
            tableObj.GetDataFromObject(item);
            bool isNew = false;
            string sql = "Select 1 From LogStatisticLabels Where ID = " + item.ID;
        ...
        {
            LogStatisticLabelModels item = new LogStatisticLabelModels();
            SQLData.ParameterCollection paraList = new SQLData.ParameterCollection();
            paraList.Add("@ID", ID);
            paraList.Add("@ShowStatus", ShowStatus);
            SQLData.Database db = new SQLData.Database(WebInfo.Conn);
            string updateSQL = " UPDATE LogStatisticLabels SET ShowStatus=@ShowStatus WHERE ID=@ID ";
            db.ExecuteNonQuery(updateSQL, paraList);
        }
        public static IEnumerable<LogStatisticLabelModels> GetItems(LogStatisticLabelSearchModels search, int pageSize, int pageIndex, out int recordCount) {
            List<LogStatisticLabelModels> items = new List<LogStatisticLabelModels>();
            if (search == null) {
                recordCount = 0;
                return items;
            }

            string sql = "Select *  From LogStatisticLabels  Where 1=1 {0} Order By LabelDate Desc";
            ...
            if (datas != null)
            {
                ...
                {
                    items.Add(new LogStatisticLabelModels
                    {
                        ID = (long)dr["ID"],
        ...
        public static IEnumerable<LogStatisticLabelModels> GetShowLabelLine(DateTime startDate, DateTime endDate)
        {
            List<LogStatisticLabelModels> items = new List<LogStatisticLabelModels>();
            string sql = "Select *  From LogStatisticLabels  Where ShowStatus=1 AND LabelDate>=@StartDate AND LabelDate<=@EndDate Order By LabelDate Asc";
